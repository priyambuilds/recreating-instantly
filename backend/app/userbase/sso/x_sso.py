...
async def x_login():
    url = (
        f"{settings.X_AUTH_URL}?client_id={settings.X_CLIENT_ID}"
...
@sso_router.get("/x/callback")
async def x_callback(request: Request, db: AsyncSession = Depends(db)):
    ...
    async with httpx.AsyncClient() as client:
        token_resp = await client.post(settings.X_TOKEN_URL, data=data, headers=headers)
        if token_resp.status_code != 200:
            raise HTTPException(status_code=400, detail="X token exchange failed")
        token_json = token_resp.json()
        access_token = token_json.get("access_token")
        if not access_token:
            raise HTTPException(status_code=400, detail="No access_token returned by X")

        user_headers = {"Authorization": f"Bearer {access_token}"}
        user_resp = await client.get(settings.X_USERINFO_URL, headers=user_headers)
        ...
        username = user_json.get("username") or (email.split("@")[0] if email else None)
